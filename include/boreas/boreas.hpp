...
/*!
   ...
   \param valid [out] Mask of whether azimuth data is an original sensor reasing or interpolated
   from adjacent azimuths \param fft_data [out] Radar power readings along each azimuth min=0, max=1
*/
void load_radar(
  std::string path, std::vector<int64_t> & timestamps, std::vector<double> & azimuths,
...
*/
void radar_polar_to_cartesian(
  const cv::Mat & polar_in, const std::vector<double> & azimuths_in, cv::Mat & cart_out,
  float radar_resolution = 0.0596, float cart_resolution = 0.2384, int cart_pixel_width = 640,
  bool fix_wobble = true)
{
  ...
  // polar to cart warp
  cv::remap(
    polar, cart_out, range, angle, cv::INTER_LINEAR, cv::BORDER_CONSTANT, cv::Scalar(0, 0, 0));
}
